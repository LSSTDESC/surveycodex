...
| zeropoint            |   float   |       mag       | The zeropoint value is a reference magnitude computed as the magnitude in the AB system (see [this reference][abmag_ref]) collected in one second and on the survey effective area. It takes into account the filter bandwidth and a classical atmosphere for ground based surveys, using the [`speclite`][speclite] library, at the airmass given by the survey `zeropoint_airmass`.                           |
